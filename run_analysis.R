...
if (!("reshape2" %in% rownames(installed.packages())) ) 
  ...
  # Merge the activities dataset with the mean/std values datase to get one dataset with descriptive activity names
  ...
  data_break <- melt(descrnames,id=c("activity_id","activity_name","subject_id"))
  
  # Arrange theis broken dataset according to  the average of each variable for each activity and each subject
  mean_data <- dcast(data_break,activity_id + activity_name + subject_id ~ variable,mean)
